...
// hooked same as CS:S it always spams when input isnt even moving unlike CS:S
bool GetRawMouseAccumulators(int& accumX, int& accumY, double frame_split)
{
	static int* m_mouseRawAccumX = (int*)((uintptr_t)g_InputSystem + 0x11C0); // X movement correct
	static int* m_mouseRawAccumY = (int*)((uintptr_t)g_InputSystem + 0x11C4); // Y movement correct 
	static bool* m_bRawInputSupported = (bool*)((uintptr_t)g_InputSystem + 0x11BC); // RawInputSupported flag correct

	ConMsg("GetRawMouseAccumulators: %d | %d | %d\n", *(int*)m_mouseRawAccumX, *(int*)m_mouseRawAccumY, *(bool*)m_bRawInputSupported); // works like CS:S but it always spams when no input is moved wtf??
...
// hooked same as CS:S it always spams when input isnt even moving unlike CS:S its like trying to make to the frame rate but its cutting out wtf? unlike CS:S
void GetAccumulatedMouseDeltasAndResetAccumulators(float* mx, float* my, float frametime)
{
	//Assert(mx);
	//Assert(my);

	static float* m_flAccumulatedMouseXMovement = (float*)((uintptr_t)g_Input + 0x8);  // X movement (Confirmed) correct
	static float* m_flAccumulatedMouseYMovement = (float*)((uintptr_t)g_Input + 0xC);  // Y movement (Confirmed) correct

	//static uintptr_t client = (uintptr_t)GetModuleHandle("client.dll");
	//int m_rawinput = *(int*)(client + 0x4F5EA0);

	static uintptr_t client = (uintptr_t)GetModuleHandle("client.dll");
	uintptr_t inputBase = *(uintptr_t*)(client + 0x7F9224);  // This loads `DAT_107f9224` correct
	int m_rawinput = *(int*)(inputBase + 0x30);  // Offset `0x30` holds `m_rawinput` correct


	ConMsg("GetAccumulatedMouseDeltasAndResetAccumulators: %.3f | %.3f | %d\n", *(float*)m_flAccumulatedMouseXMovement, *(float*)m_flAccumulatedMouseYMovement, m_rawinput); // works like CS:S but it always spams when no input is moved wtf??
...
{
	// ConMsg("WindowProc: %.3f\n", m_mouseSampleTime); // works like CS:S
...
void __fastcall Hooked_GetAccumulatedMouseDeltasAndResetAccumulators(void* thisptr, void* edx, float* mx, float* my)
{
	...
	//ConMsg("test: %.5f\n", mouseMoveFrameTime);  // works like CS:S
...
void __fastcall Hooked_ControllerMove(void* thisptr, void* edx, float ft, void* cmd)
{
	...
	// ConMsg("Hooked_ControllerMove: %.5f\n", mouseMoveFrameTime); // works like CS:S
...
void __fastcall Hooked_IN_SetSampleTime(void* thisptr, void* edx, float frametime)
{
	...
	// ConMsg("m_flMouseSampleTime: %.5f\n", m_flMouseSampleTime);  // works like CS:S
...
DWORD InjectionEntryPoint(DWORD processID)
{
	...
	g_Input = **reinterpret_cast<CInput***>(FindPattern("client.dll", "8B 0D ? ? ? ? 8B 01 FF 60 44") + 2); // hooked as same CS:S correct
	oGetRawMouseAccumulators = (GetRawMouseAccumulatorsFn)(FindPattern("inputsystem.dll", "55 8B EC 8B 45 08 8B 91 C0 11 00 00")); // hooked as same CS:S correct

	oWindowProc = (WindowProcFn)(FindPattern("inputsystem.dll", "55 8B EC 83 EC 24 57")); // hooked as same CS:S correct
	oGetAccumulatedMouseDeltasAndResetAccumulators = (GetAccumulatedMouseDeltasAndResetAccumulatorsFn)(FindPattern("client.dll", "55 8B EC 53 8B 5D 0C 56 8B F1 57")); // hooked as same CS:S correct
	oControllerMove = (ControllerMoveFn)(FindPattern("client.dll", "55 8B EC 56 8B F1 57 8B 7D 0C 80 BE 8C 00 00 00 00")); // hooked as same CS:S correct
	oIn_SetSampleTime = (In_SetSampleTimeFn)(FindPattern("client.dll", "55 8B EC F3 0F 10 45 08 F3 0F 11 41 1C")); // hooked as same CS:S correct

	uintptr_t tier = (uintptr_t)GetModuleHandleA("tier0.dll");
	ConMsg = (ConMsgFn)(uintptr_t)GetProcAddress((HMODULE)tier, "?ConMsg@@YAXPBDZZ"); // hooked as same CS:S correct
	Plat_FloatTime = (Plat_FloatTimeFn)(uintptr_t)GetProcAddress((HMODULE)tier, "Plat_FloatTime");

	ConMsg("Plat_FloatTime: %.5f\n", Plat_FloatTime()); // correct returns Plat_FloatTime
...
int main()
{
	SetConsoleTitle("GMod RawInput2");

	DWORD processID;
	printf("Waiting for GMod to start...\n");
	while (1)
	{
		processID = GetPIDByName("gmod.exe");  // Updated for GMod
		if (processID) break;
		Sleep(1000);
	}
	...
	while (1)
	{
		DWORD_PTR pClient = GetModuleHandleExtern(processID, "client.dll");
		if (pClient) break;
		Sleep(1000);
	}

	DWORD_PTR pGMod = GetModuleHandleExtern(processID, "gmod.exe");

	// Read command-line arguments, but don't check for -insecure since it's not needed in GMod
	DWORD_PTR* pCmdLine = (DWORD_PTR*)(FindPatternEx(g_hProcess, pGMod, 0x4000, (PBYTE)"\x85\xC0\x79\x08\x6A\x08", "xxxxxx") - 0x13); // correct loads gmod
	char* cmdLine = new char[255];
	ReadProcessMemory(g_hProcess, pCmdLine, &pCmdLine, sizeof(DWORD_PTR), NULL);
	ReadProcessMemory(g_hProcess, pCmdLine, &pCmdLine, sizeof(DWORD_PTR), NULL);
